...
function TargetDeepLinks({ target }) {
  ...
  const identifierGroups = [["TIC", mast.tic_ids ?? []], ["EPIC", mast.epic_ids ?? []], ["KIC", mast.kic_ids ?? []]];
            ...
            {identifierGroups.some(([, ids]) => ids.length) ? (
              <ul className="link-list">
                {identifierGroups.map(([kind, ids]) =>
                  ids.map((id) => (
                    <li key={`${kind}-${id}`}>
                      <ExternalTextLink href={mastIdentifierUrl(kind, id)}>{kind} {id}</ExternalTextLink>
                    </li>
                  ))
                )}
              </ul>
            ...
            {missionEntries.length ? (
              <ul className="stat-list">
                {missionEntries.map(([mission, count]) => (
                  <li key={mission}><span>{mission}</span><strong>{count}</strong></li>
                ))}
              </ul>
